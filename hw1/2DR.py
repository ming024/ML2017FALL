...
array = np.array([[]]).reshape(4, 0);#
row_count = -1;

for row in csv.reader(infile):
	if row_count % 18 == 1 or row_count % 18 == 3 or row_count % 18 == 9 or row_count % 18 == 13:#
...
for i in range (0, array.shape[1]):
	if array[2][i] == -1:
		...
		array[2][i] = (prev + next) / 2;

array = np.transpose(array);
array = np.append(array, array ** 2, axis = 1);

coefficients = np.array([]);
previousfile = open('previous_coefficients', 'r');
for row in previousfile:
	coefficients = np.append(coefficients, np.asarray(float(row)));
if len(coefficients) == 0:
	coefficients = np.zeros(4 * 2 * N + 1);#

derivatives_SS = np.zeros(4 * 2 * N + 1);#
for i in range (0, 4 * 2 * N + 1):#
...
for i in range (0, train_num):
	...
	(coefficients, learning_rate, derivatives_SS) = \
		functions.gradient_descent(array, coefficients, init_learning_rate, \
		randnum, samplenum, derivatives_SS);

	if(i % 200 == 0):
		print '\tRMSE\t' + str(functions.get_RMSE(array,coefficients));
	
sys.stdout.write('\b\b\b\b\b\b\b\b\b\b\b\b')
print functions.get_RMSE(array,coefficients);

testfile = open(sys.argv[1], 'r');
temp_array = np.array([[]]).reshape(0, N);
array = np.array([[]]).reshape(0, 4 * 2 * N);#
row_count = 0;
...
for row in csv.reader(testfile):
	...
	if row_count > 16:
		...
		temp_array = np.transpose(np.append(temp_array, temp_array ** 2, axis = 0));
		array = np.append(array, temp_array.reshape(1, 4 * 2 * N), axis = 0);#
		temp_array = np.array([[]]).reshape(0, N);	

	row_count += 1;

PM25_prediction = np.dot(array, coefficients[0:4 * 2 * N]);#
for data in PM25_prediction:
	data += coefficients[4 * 2 * N];#
...
previousfile = open('previous_coefficients', 'w+');
